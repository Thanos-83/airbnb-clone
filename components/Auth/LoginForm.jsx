...
function LoginForm() {
  ...
  const handleLogin = async (data)=>{
    console.log(data);
    // signIn('resend', {email: 'th.sbonias@gmail.com', redirect :false})
    const result = await signIn('resend', {email: 'th.sbonias@gmail.com'})
  ...
  return (
    <div>
      ...
      <div className='space-y-4'>
          <Button onClick={()=>signIn('facebook')} className='w-full flex items-center py-6 font-bold text-lg' variant="outline">
          <FaFacebookSquare className="mr-2 h-6 w-6 text-[#4267B2]" /> 
          <p className='text-center flex-1'>Continue with Facebook</p>
        </Button>
        <Button onClick={()=>signIn('google')} className='w-full flex items-center py-6 font-bold text-lg' variant="outline">
          <FaGoogle className="mr-2 h-6 w-6 text-[#EA4335]" /> 
          <p className='text-center flex-1'>Continue with Google</p>
        </Button>
        <Button onClick={()=>signIn('github')} className='w-full flex items-center py-6 font-bold text-lg' variant="outline">
