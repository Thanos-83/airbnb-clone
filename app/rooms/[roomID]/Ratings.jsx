...
function Ratings({ inDialog, reviewScores }) {
  ...
  return (
    <div
      className={`ratings_container grid grid-flow-col auto-cols-[minmax(170px,1fr)] overflow-x-auto ${
        inDialog
          ? 'divide-x gap-4 lg:divide-x-0 lg:gap-0 lg:flex lg:flex-col'
          : 'divide-x gap-4'
      } `}>
      <div className={`flex-1 ${inDialog ? 'mb-4' : 'pr-4'}`}>
        <p className='mb-2 font-medium text-xl'>Overall rating</p>
        <ul className='space-y-1'>
          <li className='flex items-center gap-2'>
            <span className='text-xs text-[#6a6a6a]'>5</span>
            <Progress value={80} className='h-1 w-full' />
          </li>
          <li className='flex items-center gap-2'>
            <span className='text-xs text-[#6a6a6a]'>4</span>
            <Progress value={10} className='h-1' />
          </li>
          <li className='flex items-center gap-2'>
            <span className='text-xs text-[#6a6a6a]'>3</span>
            <Progress value={10} className='h-1' />
          </li>
          <li className='flex items-center gap-2'>
            <span className='text-xs text-[#6a6a6a]'>2</span>
            <Progress value={null} className='h-1' />
          </li>
          <li className='flex items-center gap-2'>
            <span className='text-xs text-[#6a6a6a]'>1</span>
            <Progress value={null} className='h-1' />
          </li>
        </ul>
      </div>
